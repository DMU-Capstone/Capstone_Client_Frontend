  ...
  return (
    ...
    <SafeAreaView style={styles.container}>
      ...
      <Card style={styles.card}>
        ...
        <View style={styles.infoRow}>
          <Icon name="shower" size={20} color="#555" />
      ...
      {/* ✅ 현재 위치 표시할 Google 지도 추가 */}
      <View style={styles.mapContainer}>
        <Text style={[styles.title, { paddingBottom: 10 }]}>위치</Text>
        {location ? (
          <MapView
            style={styles.map}
            initialRegion={{
          ...
          >
            <Marker
              ...
              title="내 위치"
              description="현재 내 위치입니다."
            />
          </MapView>
        ) : (
          <Text style={{ textAlign: "center", color: "#888" }}>위치 정보를 가져오는 중...</Text>
        )}
      </View>
    </SafeAreaView>
  );
...
const styles = StyleSheet.create({
  ...
  mapContainer: { padding: 10, alignItems: "center" }, // ✅ 지도 스타일 추가
  map: { width: "100%", height: 200, borderRadius: 10 }, // ✅ 지도 크기 조정
});
